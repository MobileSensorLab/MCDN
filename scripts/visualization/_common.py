...
``save_figure`` helper that writes both an SVG (canonical, used by HTML render)
...
Also provides ``load_canonical_val_chip`` for data-dependent figures (F5/F6/
F7/F8 plus the queued F1 enhancement). The helper builds the canonical
Mayfield Tornado LOEO val dataset once per process and indexes it against the
cached ensemble probability tensor at
``outputs/ablation/baseline/Mayfield_Tornado/ensemble_probs.pt`` so chip
selection is deterministic, replayable, and class-conditionable. Mayfield was
chosen over the easier in-distribution Spatial_Block_East holdout to ground
the chapter's visual exemplars in the most-challenging operational regime
(the distant-OOD holdout that drives much of the discussion in chapters 5
and 6 around damage kinematics and footprint decoupling).
...
# Tornado distant-OOD split grounds chapter-4 visual exemplars in the most
# challenging operational regime - the same regime chapters 5 and 6 discuss
# most heavily for damage kinematics and footprint-decoupling effects. Other
# holdouts can be selected via the ``holdout`` argument on
# ``load_canonical_val_chip``; the chapter-6 mask-alignment exemplar (D1)
# uses Hurricane_Michael for its left panel and Mayfield_Tornado for its
# right panel.
_DEFAULT_HOLDOUT: Final[str] = "Mayfield_Tornado"
_BASELINE_DIR: Final[Path] = _REPO_ROOT / "outputs" / "ablation" / "baseline"


def _holdout_fold_dir(holdout: str) -> Path:
    """Return the seed_00 fold directory for the given holdout name."""

    return _BASELINE_DIR / holdout / "seed_00"


def _holdout_probs_path(holdout: str) -> Path:
    """Return the ensemble_probs.pt path for the given holdout name."""

    return _BASELINE_DIR / holdout / "ensemble_probs.pt"
...
def setup_publication_style() -> None:
    ...
    Style commitments:
        ...
        - Sans-serif font stack with DejaVu Sans as the always-available default;
    ...
    sns.set_theme(style="white", context="paper", font_scale=0.9, palette="colorblind")
    plt.rcParams.update({
...
def save_figure(fig: Figure, name: str, *, close: bool = True) -> Path:
    ...
    Each output directory is created on first call. SVG is the canonical
    ...
    figure - the chapter's ``fig-cap`` directive is canonical, and matplotlib's
    ...
    Returns:
        Path to the canonical SVG under ``outputs/images/``. The PDF sidecar
...
def save_caption(name: str, *, title: str, body: str) -> Path:
    ...
    ``outputs/images/<name>.caption.md`` (canonical) and ``doc/images/<name>.caption.md``
    ...
    Returns:
        Path to the canonical caption Markdown under ``outputs/images/``.
...
def _build_canonical_val_dataset(holdout: str) -> tuple[Any, np.ndarray, np.ndarray]:
    """Construct one holdout's val dataset and load its cached ensemble probs.

    Cached at module level via ``lru_cache(maxsize=4)`` so the (slow) manifest
    scan and polygon-load step happens once per holdout per process even when
    multiple chips are requested. ``maxsize=4`` accommodates the three baseline
    holdouts (Spatial_Block_East, Hurricane_Michael, Mayfield_Tornado) plus a
    spare. Reproduces the val-loader construction in
    ``src.postproc.ensemble.build_val_loader_from_config`` so dataset indices
    line up with the ``ensemble_probs.pt`` rows.

    Args:
        holdout: One of the ablation arm's split directory names under
            ``outputs/ablation/baseline/`` (e.g. ``"Mayfield_Tornado"``,
            ``"Hurricane_Michael"``, ``"Spatial_Block_East"``).
    ...
    if not config_path.exists():
        raise FileNotFoundError(f"Canonical config snapshot not found: {config_path}")
    cfg = yaml.safe_load(config_path.read_text(encoding="utf-8"))

    data_cfg = cfg["data"]
    valid_manifest = build_valid_manifest(
        data_dir=str(_REPO_ROOT / "data"),
        sensor_profile=data_cfg["sensor_profile"],
    )

    holdout_selection = (
        cfg.get("metadata", {}).get("holdout_selection")
        or data_cfg.get("holdout_selection", "explicit")
    )
    ...
    _holdout_name, _train_df, val_df, _selection = select_fold(
        splits=splits, holdout_event=holdout_event_arg
    )
    ...
    if not probs_path.exists():
        raise FileNotFoundError(
            f"Canonical ensemble probs cache not found: {probs_path}. "
            f"Run scripts/run_per_arm_ensembles.py first."
        )
    cache = torch.load(probs_path, map_location="cpu", weights_only=False)
    targets = cache["targets"].numpy().astype(np.int64)
    ensemble_probs = cache["ensemble_probs"].numpy().astype(np.float32)

    if len(dataset) != targets.shape[0]:
        raise RuntimeError(
            f"Dataset size ({len(dataset)}) does not match cached probs row count "
            f"({targets.shape[0]}); the canonical fold's manifest may have drifted "
            f"since the ensemble was rendered."
        )

    return dataset, targets, ensemble_probs


def load_canonical_val_pool(holdout: str = _DEFAULT_HOLDOUT) -> dict[str, Any]:
    """Load the full canonical val pool for one holdout.

    Returns the val dataset, targets, per-seed individual probs, and the
    softmax-mean ensemble probs - everything the chapter-6 decoding-rule
    figure (D2) needs to render per-seed bar charts plus an ensemble bar
    chart for one specific manifest index.

    Args:
        holdout: Split directory name under ``outputs/ablation/baseline/``.
    ...
    dataset, targets, ensemble_probs = _build_canonical_val_dataset(holdout)
    cache = torch.load(
        _holdout_probs_path(holdout), map_location="cpu", weights_only=False
    )
    individual_probs = cache["individual_probs"].numpy().astype(np.float32)
    seeds = list(cache["seeds"])
...
def load_canonical_val_chip(
    *,
    ...
    """Load a representative chip from one canonical baseline LOEO val set.

    The dataset is the same one MCDN's baseline arm validated on at the
    requested holdout (``outputs/ablation/baseline/<holdout>``); chip indexing
    is aligned to the cached ensemble probabilities so any chip we render also
    has its ensemble softmax available for downstream display.

    Default holdout is ``Mayfield_Tornado`` (the distant-OOD split, used by
    F1 / F5 / F6 / F7 / F8 in chapter 4); the chapter-6 mask-alignment
    exemplar (D1) overrides to ``Hurricane_Michael`` for its left panel.
